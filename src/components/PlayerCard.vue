...
const scores = computed(() => playerScores(props.data.games, props.data.assets));
const profile = computed(() => playerProfile(props.data.games, props.data.assets));
const names = {
  ceiling: '고점력',
  stability: '안정성',
  survival: '순방력',
...
<template>
  <section class="panel player-card">
    ...
    <h3>선호 챔피언 TOP 3</h3>
    <p v-for="r in profile.units.top.slice(0, 3)" :key="r.id" class="small">
      {{ name('unit', r.id, r.set) }} · {{ r.count }}경기 <span v-if="!r.enough">(표본 부족)</span>
    </p>
    <p v-if="!profile.units.top.length" class="small muted">기록 부족</p>
    ...
    <details class="method">
      ...
      <p>{{ profile.reason }}</p>
      <p>
        성향은 20경기 이상, 보드·활성 특성 기록 80% 이상 필요합니다. 최소 2~3개의 점수와 사용
        집중도·실제 성적을 조합한 규칙 기반 칭호이며 Riot 공식 실력·백분위가 아닙니다.
      </p>
    </details>
  </section>
</template>
