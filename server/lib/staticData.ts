...
function staticVersion(v = 'latest') {
  return /^(latest|\d+\.\d+)$/.test(v) ? v : 'latest';
}
async function catalog(requestedVersion = 'latest'): Promise<StaticCatalog> {
  const version = staticVersion(requestedVersion);
...
export async function loadGameAssets(
  ...
  const source = await catalog(version);
