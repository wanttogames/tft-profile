...
describe('server-rendered 30-game UI', () => {
  it('renders ten match details initially, without reducing the thirty-game dataset', async () => {
    ...
    expect(html).toContain('10 / 30경기');
    expect(data.games).toHaveLength(30);
  });
  it('renders the preference table and sample rules using all thirty games', async () => {
    const html = await renderToString(
      createSSRApp(PreferencePanel, { data: demoPlayer(), kind: 'unit' }),
    );
    expect(html).toContain('선호 챔피언 TOP 10');
    expect(html).toContain('분석 30경기');
