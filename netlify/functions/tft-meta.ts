...
export default async function handler(request: Request): Promise<Response> {
  ...
  const min = Number(process.env.MIN_SAMPLE_SIZE ?? '10');
  ...
  if (!url || !secret)
    return reply(
      {
        message:
          '메타 데이터 연결이 설정되지 않았습니다. Netlify의 SUPABASE_URL / SUPABASE_SECRET_KEY를 설정해 주세요.',
      },
      503,
    );
  let base: string;
  ...
  const key = `${base}|${min}|${kind}|${sort}|${page}`;
  ...
  if (!pending.has(key)) {
    const job = (async (): Promise<MetaData> => {
      ...
      const data: MetaData = {
        ...
        page,
        assets: await loadMetaAssets(requests),
      };
