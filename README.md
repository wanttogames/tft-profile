...
- PLAY DNA: 고점력, 안정성, 순방력, 후반 운영력, 덱 다양성, 유연성, 보드 완성도, 최근 폼을 최근 최대 50경기의 관측 가능한 결과와 최종 보드로 계산합니다. 각 공식과 고정 정규화 기준은 UI 도움말과 `playerScores.ts`에 있습니다. 최소 유효 표본 5개가 필요하며, 정적 데이터로 분류할 수 없는 아이템은 완성도 계산에서 제외합니다.
