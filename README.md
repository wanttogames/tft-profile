...
증강 정보 0경기를 곧바로 파싱 실패라고 판단하지 않습니다. `info.participants`에서 ACCOUNT-V1으로 얻은 PUUID와 일치하는 객체를 선택한 뒤, 실제 응답에서 확인한 `augments: string[]`만 선택 기록으로 사용합니다. 알 수 없는 증강 관련 필드는 추측해서 변환하지 않습니다.

- **정상 기록**: 문자열 배열의 모든 슬롯을 보존합니다. 세 개로 자르거나 부족한 슬롯을 채우지 않습니다.
- **선택 기록 없음**: 실제 빈 배열. 통계 분모에는 포함합니다.
- **실제 데이터 없음(API 필드 누락·null)**: 응답에 선택 기록이 제공되지 않았습니다. 게임에서 증강을 선택하지 않았다는 뜻이 아닙니다. 증강 통계에서 제외합니다.
- **파싱 오류**: 값의 형식이 잘못됐거나 검증되지 않은 증강 관련 필드가 있습니다. 해당 경기의 증강 통계만 제외하고 일반 전적 통계는 유지합니다.

`.env`에 `TFT_DEBUG_AUGMENTS=1`을 설정하고 `npm run dev:api`로 실행하면 서버 터미널에서 **검색 참가자의 증강 관련 키·값과 파싱 상태만** 확인할 수 있습니다. 운영 환경에서는 이 옵션을 켜도 로그를 남기지 않습니다. 개발 로그를 보기 전에 서버를 재시작하고 다시 검색하세요(검색 캐시 최대 2분).

실제 문제가 발생한 응답을 저장하려면 `.env`의 `RIOT_API_KEY`를 설정한 뒤 실행합니다.

```bash
npm run capture:match -- "게임이름#KR1"
# 특정 경기: 마지막 인자로 Match ID 입력
npm run capture:match -- "게임이름#KR1" "KR_경기ID"
```

ACCOUNT-V1 → PUUID → 최근 경기 ID → Match-V1을 순서대로 호출하며 `diagnostics/riot-match-날짜.json`과 출처 메타데이터를 저장합니다. 기본 캡처는 진단용 최근 1경기이며 서비스의 최근 50경기 조회 설정을 바꾸지 않습니다. 파일에는 PUUID와 Riot ID를 익명화하고, 증강 관련 원본 키·값은 보존합니다. `diagnostics/`는 Git에서 제외됩니다. 재현용 JSON을 전달할 때 API 키나 `.env`는 첨부하지 마세요.

2026-09-12 확인 범위: 공식 DTO 문서에는 증강 필드가 명시되지 않았고, 공개된 세트 18 PBE 참가자 사례에도 해당 키가 없었습니다. 증강 3개 추출은 공개된 과거 실제 응답으로 검증했습니다. 사용자 계정의 최신 응답은 아직 확보하지 못했으므로, 현재 31경기의 원인이 파싱인지 API 미제공인지 확정하지 않았습니다. 증강을 제공하지 않는 응답에서 정적 데이터만으로 선택 기록을 복원할 수는 없습니다.
